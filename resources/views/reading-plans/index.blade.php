                                ...
                                @foreach($readingPlans as $plan)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <!-- 💡 修正ポイント①：\Carbon\Carbon::parse を挟んで安全に変換 -->
                                            {{ \Carbon\Carbon::parse($plan->target_date)->format('Y-m-d') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <!-- 💡 修正ポイント②：完了日時（completed_at）も値がある場合だけ安全にパースします -->
                                            {{ $plan->completed_at ? \Carbon\Carbon::parse($plan->completed_at)->format('Y-m-d') : '-' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span
